...
const RegisterConsent = () => {
  ...
  const [policy, setPolicy] = useState(ConsentPolicy.AI_ALLOWED);
      ...
      <div className="max-w-xl mx-auto space-y-6">
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-8 text-center space-y-4">
          ...
          <p className="text-emerald-800">
            Your image consent policy has been permanently recorded.
          </p>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 space-y-8">
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            >
              <div
                className={`mt-0.5 w-5 h-5 rounded-full border flex items-center justify-center ${policy === ConsentPolicy.AI_ALLOWED ? "border-emerald-500 bg-emerald-500" : "border-slate-300"}`}
              >
                {policy === ConsentPolicy.AI_ALLOWED && (
              ...
              <div className="flex-1">
                ...
                <p className="text-slate-500 text-sm">
                  Full training usage permitted.
                </p>
              </div>
              <Globe
                className={`w-5 h-5 ${policy === ConsentPolicy.AI_ALLOWED ? "text-emerald-500" : "text-slate-300"}`}
              />
            </button>
            ...
            >
              <div
                className={`mt-0.5 w-5 h-5 rounded-full border flex items-center justify-center ${policy === ConsentPolicy.AI_RESTRICTED ? "border-amber-500 bg-amber-500" : "border-slate-300"}`}
              >
                {policy === ConsentPolicy.AI_RESTRICTED && (
                  <div className="w-2 h-2 bg-white rounded-full" />
                )}
              </div>
              <div className="flex-1">
                ...
                <p className="text-slate-500 text-sm">
                  Non-generative use only.
                </p>
              </div>
              <Shield
                className={`w-5 h-5 ${policy === ConsentPolicy.AI_RESTRICTED ? "text-amber-500" : "text-slate-300"}`}
              />
            </button>
            ...
            >
              <div
                className={`mt-0.5 w-5 h-5 rounded-full border flex items-center justify-center ${policy === ConsentPolicy.PLATFORM_ONLY ? "border-red-500 bg-red-500" : "border-slate-300"}`}
              >
                {policy === ConsentPolicy.PLATFORM_ONLY && (
                  <div className="w-2 h-2 bg-white rounded-full" />
                )}
              </div>
              <div className="flex-1">
              ...
              <Lock
                className={`w-5 h-5 ${policy === ConsentPolicy.PLATFORM_ONLY ? "text-red-500" : "text-slate-300"}`}
              />
            </button>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 p-4 bg-blue-50 text-blue-800 rounded-lg text-sm">
          ...
          <p>
            By registering, you affirm you hold the necessary rights to this
            image. This action creates a permanent verification record.
          </p>
        </div>
      ...
      <div className="mt-8 bg-slate-100/50 p-4 rounded-xl">
        <details>
          ...
          <p className="mt-2 text-sm text-slate-600 leading-relaxed">
            Images are not stored. We generate a perceptual hash (dHash) that
            represents the visual structure of your image. This hash is stored
            in our public registry along with your selected policy. AI labs and
            platforms can query this registry to verify usage rights without
            needing access to the original image.
          </p>
        </details>
      </div>
    </div>
