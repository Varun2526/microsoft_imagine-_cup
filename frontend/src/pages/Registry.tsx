...
const Registry: React.FC = () => {
    ...
    const getPolicyBadge = (policy: string) => {
        ...
        return (
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${(styles as { [key: string]: string })[policy] || 'bg-slate-100 text-slate-800'}`}>
                {policy.replace('AI_', '')}
            </span>
        );
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Consent Registry</h1>
                    <p className="text-slate-500">Global ledger of registered image policies</p>
                </div>
                <button
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-500">Loading registry...</td></tr>
                            ) : records.length === 0 ? (
                                    ...
                                    <tr key={record.id} className="hover:bg-slate-50 transition-colors">
                                        <td className="px-6 py-4 font-mono text-slate-600">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span title={record.image_hash}>{record.image_hash.substring(0, 12)}...</span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="text-slate-700 capitalize">{record.status}</span>
                                        </td>
